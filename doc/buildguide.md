...
|R3|1.5u x 2|Controlキー、Enterキー|
|R4|1.75u|Shiftキー|
|R4|1.25u|Altキー|
|R4|2u x 2, 2.75u|スペースキー|
...
RP2040-Tinyについては[公式ドキュメント](https://www.waveshare.com/wiki/RP2040-Tiny)を参照ください。

まず、RP2040-Tinyのメインボードとアダプタボードを付属のフラットケーブルで接続します（公式ドキュメントに写真入りで説明があります）。その後、お使いのコンピュータのUSB 2.0以上に対応したUSBコネクタと、RP2040-Tinyを接続してください。RP2040-Tinyが正常であれば、`RPI-RP2`というドライブがマウントされると思います。
...
基板裏面右下のパッドにあわせてRP2040-Tinyメインボードを直接置き、はんだ付けをしてください。ピンヘッダは不要です。
...
オプション品のスイッチパッドを購入している場合は、この段階でスイッチパッドを基板の**表面**に設置してください。穴の位置を合わせて単純に乗せるだけで良いです。この上からスタビライザーやスイッチを取り付けることで固定します。
...
フラットケーブルは多少曲げたり折ったりしても問題ありませんが、過度にねじれたり何度も折り曲げたりすると中の銅線が断線しますので注意してください。
